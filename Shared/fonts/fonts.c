...
text font_get(const font* font, const char* val) {
	...
	//d.s = malloc(sizeof(bytestream) * d.n);
	for (i = 0; i < d.n; ++i) {
		c = val[i];
		c = (c < ' ' || c > '~')
